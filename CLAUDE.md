...
- **通知先 `notificator` は Connect プロトコルの JSON を素の HTTP POST で叩く**（`notificator.notification.svc.cluster.local:50051`）。`grpcio` や proto のコード生成は不要。バックエンドが LINE なので送れるのは**単一の短い文字列**。候補一覧は `notify.max_listed`（20件）以下なら全件載せ、超えたら一覧ごと省略して超過を明示する（上位N件の切り出しは、その順序が評価の順位に読めるのでやらない）。
