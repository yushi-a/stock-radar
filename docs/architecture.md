...
### 未決：実データを見てから決める（Phase 2a）

タグの優先順位リストだけでは正規化は決まらない。以下は `companyfacts` の実データを見て確定する。

| 論点 | 内容 |
|---|---|
| **A. 年次レコードの選定** | 同一会計年度の値が複数回出る（元の10-K + 翌年の比較欄での再掲）。修正再提出で値が変わることがあり、「当時報告された値」と「最新の値」のどちらを採るかを決める必要がある。決算期変更により12ヶ月でない「年度」も発生し、そのまま3年CAGRを計算すると壊れる（`end - start` でガードする方針の妥当性を実データで確認する）。BS項目は時点値（`end` のみ）、PL/CF項目は期間値（`start`+`end`）で扱いが違う |
| **B. 四半期の YTD → 3ヶ月変換** | トラックBの「直近四半期YoY」には3ヶ月値が必要だが、多くの企業は YTD でしか報告しない。Q2 = YTD(Q2) − YTD(Q1)、Q4 = 通期 − YTD(Q3) のような引き算ルールを確定する |
| **C. 複数クラス株の株数** | `dei:EntityCommonStockSharesOutstanding` はクラスごとに出るため、合算しないと時価総額が過小になる。ただしクラスで株価が違うので単純合算 × 片方の株価も厳密には誤り。**要検証の仮説：`companyfacts` は軸（dimension）付きファクトを含まない可能性があり、その場合はクラス別株数を別ルートで取る必要がある** |
| **D. 指標ごとの欠損率** | `GrossProfit`・設備投資・`OperatingIncomeLoss` の欠損率を zip 全体で測る。高ければ**スクリーニング条件そのものの見直しが必要**（例：粗利率をハードフィルタにできない） |

詳細と進め方は `docs/implementation-plan.md` の Phase 2a を参照。
...
定常的に必要な容量は **5〜7GB 程度**（zip 2種を1世代ずつ + DuckDB + 作業領域）。
