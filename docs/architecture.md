...
| 出力 | CSV + 通知（notificator）。CSV は Google Drive にも上げる | 同一クラスタ内にデプロイ済みの自前アプリ `notificator` に送る。Service 名で到達できる。インターフェースは実装時に確認する。CSV を Drive に上げるのは評価スキルを claude.ai で動かすため（2026-09-26 決定、「CSV の取り出し」） |
...
処理順が設計の要。**株価取得を最後に回す**ことでネットワーク負荷が最も重い工程の対象を減らす。
設計時は「半減」を見込んでいたが、2026-09-21 の実測では **3,560 → 243 銘柄（6.8%）**まで落ちた。
...
② 財務指標を全社計算
   ...
   → 株価が要らない条件をすべて当てて足切り（自己資本・売上・成長率・利益率・
      粗利率・ROA/ROE・資産成長率−EBIT成長率・財務余力）
   3,560ティッカー → 243（2026-09-21 実測。設計時の見込みは約1,400）
                                                [ネットワーク1回・あとはローカル処理]

③ 株価を取得
   ②の通過分だけ yfinance で日次1年3ヶ月分（差分追記）
   対象の決定は screen.runner.prescreen_tickers（株価が要らない条件をすべて当てる）
   → 時価総額・平均売買代金・52週高安
   243銘柄                                       [ここだけが重い。初回フル、以降は差分]
...
`prices_daily` のフル取得には18分（243銘柄。2026-09-21 実測）かかり、`fundamentals` も
元の `companyfacts.zip` を1世代しか残さないため、黙って捨ててよいテーブルではない。
...
②で対象を減らしておくこと、直列化（`threads=False`）、十分なスリープが主役になる。

**2026-09-21 実測：足切り後の 243 銘柄を 18分05秒（1件あたり約4.5秒）で完走し、429 は0件。**
「丸1日」は許容量であって所要時間ではない。時間予算（`max_wall_clock_sec: 72000`）は
余裕をもったままにしておく。上場銘柄が増えても、閾値を緩めても、打ち切られずに済む。
...
対策は別ファイルへの書き直し。実測で **201MB → 51MB を1.1秒**で、
データもスキーマバージョンもそのまま残る。`fetch-facts` の最後で必ず実行する。

**`COPY FROM DATABASE` は使えない**（2026-09-22、issue #49）。テーブルを外部キーの順に
並べてくれず、`screen_results` を `screen_runs` より先に写して FK 違反で落ちる。
スクリーニングを1回でも実行した後の DB では必ず再現し、**週次運用では2回目以降が毎回失敗する**。
親から順に自分で `INSERT` している。参照を持つテーブルを足すときは `ALL_TABLES` の
並び（親 → 子）を崩さないこと。`screen_run_id_seq` の続き番号も引き継ぐ必要がある
...
- **載せるのは要約と候補一覧だけ**（実行日、ユニバース件数、トラック別の通過件数、
  `price_coverage`、候補一覧、CSV の URL）。判断（スコア・期待倍率）は載せない
- **候補一覧は `notify.max_listed`（20件）まで載せ、あふれた分は「ほかN件は省略」と
  書いてから落とす**（2026-09-22 決定）。黙って切ると届いた一覧が候補の全部だと
  読めてしまうため。実測の候補数は21件前後
- **一覧の見出しは「上位」と書かない。** 並びはタイミング加点順であって評価の順位では
  ないので、順位として読ませない
- **CSV の添付はこの経路ではできない。** CSV は Google Drive に上げ、通知には URL を載せる
  （次節）。アップロードに失敗したら PVC 上のパスと失敗の理由を載せる
...
## CSV の取り出し（Google Drive）

2026-09-26 決定（#48）。CSV は PVC（local-path）にも書くが、それだけでは
`/var/lib/rancher/k3s/storage` が root 専用で見るのに `sudo` が要り、CronJob の Pod は
終了後に消えるので `kubectl cp` も使えない。評価スキルは **claude.ai / クラウド**で動かすので、
手元のファイルでは届かない。そこで Drive の非公開フォルダに上げ、通知に URL を載せる。
claude.ai からは Drive コネクタで読める。

実装は `src/stock_radar/output/drive.py`。Google のクライアントライブラリは使わず、httpx で REST を数本叩く。

### 選んだ理由

| 案 | 見送った理由 |
|---|---|
| hostPath でホーム配下に書き出す | 評価スキルを claude.ai で動かすので、手元にあっても届かない |
| Cloudflare R2 / S3 互換 | claude.ai から読ませるには署名付き URL を毎回貼る運用になる |
| 静的配信（PVC を読む HTTP サーバ） | 常駐が1つ増え、Cloudflare Access 等の認証も要る |
| メール添付 | 認証の手間は Drive と同じで、ファイルがメールに散らばる |
| 別リポジトリにコミット | 「データをリポジトリにコミットしない」方針と衝突する |

**非公開のまま自分のアカウントに置く**ので、J-Quants の「分析結果の継続的な第三者提供」にも当たらない
（日本株を足しても使い続けられる）。公開 URL に置く案はここで使えなくなりうる。

### 認証：OAuth のリフレッシュトークン

- **個人アカウントではサービスアカウントが使えない。** サービスアカウントには保存容量が無く、
  共有されたフォルダにも作れない（回避には Workspace の共有ドライブが要る）。本人として書く
- **スコープは `drive.file`。** このアプリが作ったファイルにしか触れない。審査も要らない
- **OAuth アプリは「本番」にしておく。** 「テスト」のままだとリフレッシュトークンが7日で失効し、
  週次ジョブが毎回失敗する
- 同じ日の再実行では同名ファイルの中身を差し替える（重複させない）

### 上げ先のフォルダ

`drive.file` では手で作ったフォルダはアプリから見えない（中にファイルを作れない）。
**使えるのはアプリが作ったフォルダだけ**なので、決め方は次の2通り（2026-09-26 決定、#60）。

| 環境変数 `GOOGLE_DRIVE_FOLDER_ID` | 挙動 |
|---|---|
| 入っている | **その ID のフォルダに上げる。** Drive の画面で名前を変えても移動しても追える。見つからない・フォルダでない・ゴミ箱にある場合は**失敗にし、名前で探すほうへは戻らない**（戻ると、ID を入れた意図と違う場所に黙って上がる）。失敗は run を落とさず通知に書く |
| 空か未設定 | `drive.folder_name`（`stock-radar`）で探し、無ければ作る。移動はしてよいが、**名前を変えると別のフォルダを作り直す** |

ID は、ID 無しで一度上げてアプリにフォルダを作らせ、Drive でそのフォルダを開いたときの
URL（`https://drive.google.com/drive/folders/<ID>`）から取る。手で作ったフォルダの ID を入れても
「見つからない」で失敗する。ID は秘密ではないが、他の資格情報と同じく Secret（SOPS）から渡す。

### 失敗しても run を落とさない

通知と同じ方針。CSV は PVC に残っているので後から上げ直せる。Job を失敗にすると
CronJob の再試行でパイプライン全体（20分）をやり直すことになり割に合わない。
代わりに通知の最終行に `CSV: <PVC のパス>（Drive へのアップロード失敗: <理由>）` と書く。
トークンの失効は放っておいても直らないので、毎週の LINE で目に入るようにする。

### 初回のセットアップ（手作業）

1. Google Cloud Console でプロジェクトを作り、**Google Drive API を有効にする**
2. OAuth 同意画面を作り（User type は外部）、スコープに `.../auth/drive.file` を足す。
   **公開ステータスを「本番」にする。** ブランディング画面では任意欄に見えるが、外部向けの本番公開には
   **ホームページ URL とプライバシー ポリシー URL が必須**（未入力だと「アプリを公開」がグレーアウトし、
   理由はボタンのツールチップにしか出ない。2026-09-26 に実際に踏んだ）。URL のドメインは承認済みドメインにも足す。
   ロゴは入れない（入れるとブランド確認が要る）
3. 認証情報で OAuth クライアント ID を**デスクトップアプリ**として作る
4. `.env` に `GOOGLE_OAUTH_CLIENT_ID` / `GOOGLE_OAUTH_CLIENT_SECRET` を書き、
   `uv run stock-radar drive-auth` を実行する。表示された URL をブラウザで開いて同意すると、
   `GOOGLE_OAUTH_REFRESH_TOKEN=...` が表示される
   - `127.0.0.1` の一時ポートで戻りを待つので、ブラウザは同じマシンで開く。
     別のマシンで開いたときは、戻り先（接続できないページ）のアドレスバーの URL をコピーし、
     `drive-auth` を動かしているマシンで `curl '<その URL>'` を叩けば受け取れる
5. 3つの値を `yushi-a/helm` の `charts/stock-radar` の Secret（SOPS）に入れる
6. `uv run stock-radar screen`（`--no-notify` 可）で1回上げ、Drive の `stock-radar` フォルダに
   CSV ができること、claude.ai の Drive コネクタから読めることを確かめる
7. （任意）フォルダを ID で固定する場合は、6で作られたフォルダの ID を Secret（`drive.folderId`）にも入れる。
   以後は名前を変えても移動してもよい（前節「上げ先のフォルダ」）
