...
- **載せるのは要約と候補一覧だけ**（実行日、トラック別の通過件数、`price_coverage`、
  候補一覧、CSV のパス）。判断（スコア・期待倍率）は載せない
- **候補一覧は全件載せるか、1件も載せないか**（2026-09-22 決定）。`notify.max_listed`
  以下なら全件、超えたら一覧ごと省略して超過を明示する。上位N件だけを載せると、その
  順序（タイミング加点順）が評価の順位に読めてしまうため。実測の候補数は21件前後
