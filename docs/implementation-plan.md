...
- 直列化（`threads=False`）＋リクエスト間スリープ、指数バックオフ、429 は `fetch_failures` へ積んで次の銘柄へ進む
...
1. K3s CronJob 化（実行時刻をここで決定。推奨は土曜朝 JST）
   ...
   - **`concurrencyPolicy: Forbid` を設定する**（DuckDB は同時書き込み不可。Phase 3 が30〜40分かかるため重複起動は現実に起こりうる）
